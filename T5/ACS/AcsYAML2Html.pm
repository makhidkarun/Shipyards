...
#   $opsQuality = "<p>$opsQuality</p>" if $opsQuality;
   ...
   # NOTE: used to include width in table tag:  width='615px'>
   my $out =<<EOHEADER;
<html>
<head>
<style type='text/css'>
...
<hr />
<table rules='all' cellpadding='2' width='500px' >
EOHEADER
...
sub encodeHtml
{
   ...
   # NOTE: used to include TL after Tons as : <td>$tl</td>
      $body .=<<EOLN;
...
#   my $notes    = shift;
   ...
   # NOTE: used to include TL after Tons with: <td><i>TL</i></td>
   my $hdr =<<EOLN;

<tr bgcolor='d0d0d0'>
<th width=90px rowspan='$rowspan'><font size='-1'>$title<br />$subtitle</font></th>
   <td><i>Tons</i></td>
   <td><i>Component</i></td>
   <td><i>MCr</i></td>
EOLN
